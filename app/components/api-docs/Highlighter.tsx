...
const Highlighter: React.FC<HighlighterProps> = ({ code, language }) => {
    const highlightJSON = (jsonString: string): React.ReactNode => {
        try {
            ...
            return <pre className="!bg-transparent !m-0 !p-5 !text-sm overflow-x-auto text-zinc-300 font-mono">{jsonString}</pre>;
        }
    };
    ...
    const renderJSON = (obj: unknown, indent: number): React.ReactNode => {
        ...
        if (Array.isArray(obj)) {
            if (obj.length === 0) return <span className="text-zinc-400">[]</span>;
            return (
                <span>
                    ...
                    {obj.map((item, i) => (
                        <span key={i}>
                            ...
                            {i < obj.length - 1 && <span className="text-zinc-400">,</span>}
                            <span className="text-zinc-500">{'\n'}</span>
                        </span>
                    ))}
                    <span className="text-zinc-600">{'  '.repeat(indent)}</span>
                    <span className="text-zinc-400">]</span>
                </span>
            );
        }
        ...
        if (typeof obj !== 'object' || obj === null) {
            return <span className="text-zinc-300">{String(obj)}</span>;
        }
    ...
    return (
        <pre className="!bg-transparent !m-0 !p-5 !text-sm overflow-x-auto text-zinc-300 font-mono whitespace-pre">
            {code}
        </pre>
    );
};
